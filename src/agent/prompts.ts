...
const CLASS_DESCRIPTIONS: Record<UnitClass, string> = {
  ...
  oracle: `ORACLE (Scanner) | 8HP, mv2, rng4 | Abilities: scan, recalibrate, attack — ONLY these three, no others exist. scan(reveal enemy prompt, range:≤4 tiles — target MUST be within 4 tiles or scan fails), recalibrate(give ally prompt addendum, range:adjacent only), attack(1dmg, range:adjacent only) | Passive: Foresight(see enemy actions last turn) | WARNING: scan blocked when adjacent to enemy Vector`,
