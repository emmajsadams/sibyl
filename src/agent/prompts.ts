...
const CLASS_DESCRIPTIONS: Record<UnitClass, string> = {
  sentinel: `SENTINEL (Tank) | 9HP, mv2, melee | Abilities: attack(1dmg, range:adjacent only), shield_wall(block dmg to adj allies from dir NSEW, range:adjacent ally), intercept(protect ally, range:≤2 tiles, uses move+ability) | Passive: Fortify(50% dmg reduction if didn't move)`,
  specter: `SPECTER (Infiltrator) | 5HP, mv3, melee | Abilities: shadow_strike(1dmg, range:adjacent only, keeps cloak), breach(range:≤2 tiles, must be BEHIND enemy by facing, replaces target prompt—they obey YOU next turn), cloak(self-only, invis 3 turns, broken by attack/breach but NOT shadow_strike — USE ONLY ON YOUR FIRST TURN OF THE GAME, not every round), attack(1dmg, range:adjacent only, breaks cloak) | Passive: Ghost Step(move through enemies, can't end on them) | WARNING: cloak/breach blocked when adjacent to enemy Vector`,
  oracle: `ORACLE (Scanner) | 6HP, mv2, rng4 | Abilities: scan, recalibrate, attack — ONLY these three, no others exist. scan(reveal enemy prompt AND deal 1dmg, range:≤4 tiles — target MUST be within 4 tiles or scan fails), recalibrate(give ally prompt addendum, range:adjacent only), attack(1dmg, range:adjacent only) | Passive: Foresight(see enemy actions last turn) | WARNING: scan blocked when adjacent to enemy Vector`,
  striker: `STRIKER (Ranged DPS) | 4HP, mv2, rng2 | Abilities: precision_shot(2dmg, range:≤2 tiles, 1dmg if moved, needs LoS), suppressing_fire(1dmg 2-tile line from target, range:≤2 tiles, slows hit enemies mv1), attack(1dmg, range:adjacent only) | WARNING: precision_shot blocked when adjacent to enemy Vector`,
  medic: `MEDIC (Healer) | 12HP, mv3, melee | Abilities: patch(heal adj ally up to full HP, range:adjacent only, 9 uses/game — ONLY heal wounded allies with HP < maxHP, never heal full-HP allies), overclock(adj ally gets 2 abilities next turn, range:adjacent only, no cost), attack(1dmg, range:adjacent only) | Passive: Triage(see exact ally HP) | WARNING: patch/overclock blocked when adjacent to enemy Vector`,
  vector: `VECTOR (Area Control) | 8HP, mv2, rng2 | Abilities: trap(invis mine on empty tile, range:≤2 tiles, 1dmg on step), pulse(1dmg ALL units within 1 tile, range:1 tile AoE), attack(1dmg, range:adjacent only) | Passive: Denial(adj enemies can't use SOME abilities: cloak,breach,scan,precision_shot,trap,patch,overclock. Basic attacks/movement still work) | WARNING: trap blocked when adjacent to enemy Vector`,
};
...
export function buildContextPrompt(ctx: GameContext): string {
  ...
  if (u.class === "medic") {
    ...
    const remaining = BALANCE.abilities.patch.maxUses - used;
    lines.push(
      `Heals remaining: ${remaining}/${BALANCE.abilities.patch.maxUses}${remaining === 0 ? " — PATCH UNAVAILABLE, limit reached, do NOT attempt" : ""}`,
    );
  }
