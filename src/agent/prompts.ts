...
const CLASS_DESCRIPTIONS: Record<UnitClass, string> = {
  ...
  oracle: `ORACLE (Scanner) | 8HP, mv2, rng4 | Abilities: scan(reveal enemy prompt, range:≤4 tiles — target MUST be within 4 tiles or scan fails), recalibrate(give ally prompt addendum, range:adjacent only), attack(1dmg, range:adjacent only) | Passive: Foresight(see enemy actions last turn) | WARNING: scan blocked when adjacent to enemy Vector`,
...
export function buildContextPrompt(ctx: GameContext): string {
  ...
  if (u.class === "medic") {
    ...
    lines.push(`Heals remaining: ${remaining}/3${remaining === 0 ? " — PATCH UNAVAILABLE, limit reached, do NOT attempt" : ""}`);
  }
  ...
  if (u.class === "specter") {
    ...
    lines.push(`Breaches used: ${breachesUsed}/2${breachCapped ? " — BREACH UNAVAILABLE — limit reached, do NOT attempt" : ""}`);
    ...
    if (ctx.round === 1 && !isCloaked) {
      ...
      lines.push(`TIP: Already cloaked. Do NOT re-cloak. Use your cloak to get behind enemies for breach/shadow_strike.`);
    } else if (ctx.round > 1 && !isCloaked) {
      lines.push(`TIP: Round ${ctx.round} — do NOT cloak again unless tactically critical. Focus on offense.`);
    }
  }
  ...
  const cloakEffect = u.statusEffects.find((e) => e.type === "cloaked") as { type: "cloaked"; turnsLeft: number } | undefined;
  if (cloakEffect) {
  ...
  if (ctx.allies.length > 0) {
    lines.push(
      "Allies: " +
        ctx.allies
          .map((a) => {
            const hp = a.hp !== undefined ? `HP:${a.hp}/${u.class === "medic" ? (() => { const stats = UNIT_STATS[a.class]; return stats.maxHp; })() : "?"}` : a.status;
            const dist = Math.abs(a.position.x - u.position.x) + Math.abs(a.position.y - u.position.y);
            let extra = "";
  ...
  for (const v of enemyVectors) {
    ...
    if (dist <= 1) {
      lines.push(`[⚠ WARNING: Adjacent to ${v.name}'s DENIAL zone — abilities may be BLOCKED (only basic attack/movement work)]`);
    } else if (dist <= 3) {
      lines.push(`[CAUTION: ${v.name}(vector) is ${dist} tiles away — avoid moving adjacent or abilities will be blocked]`);
    }
  }
  ...
  if (ctx.enemies.length > 0) {
    lines.push(
      "Enemies: " +
        ctx.enemies
          .map((e) => {
            const dist = Math.abs(e.position.x - u.position.x) + Math.abs(e.position.y - u.position.y);
            let extra = "";
            if (u.class === "oracle") {
              ...
              if (ctx.scannedEnemies && ctx.scannedEnemies[e.id]) {
                ...
                extra = dist <= 4 ? ` [in scan range, dist=${dist}]` : ` [OUT OF SCAN RANGE, dist=${dist} > 4]`;
              }
            } else {
              extra = ` [dist=${dist}]`;
            }
            ...
            if (e.class === "vector") {
              extra += dist <= 1 ? ` [DENIAL ACTIVE — you are adjacent]` : ` [denial range: adjacent]`;
            }
